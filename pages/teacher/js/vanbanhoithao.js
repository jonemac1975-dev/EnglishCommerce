...
/* =========================
   DOM
========================= */
let bgVanBan, bgMp3,bgMp32, bgMp4, bgYoutube,
    bgNoiDung, bgNgay,
...
function getDOM() {
  ...
  bgVanBan = document.getElementById("bgVanBan");
...
function collectData() {
  ...
  if (!bgVanBan.value) {
    showToast("Thiếu tên văn bản");
    return null;
  }

  return {
    title: bgVanBan.value,
    ngay: bgNgay.value || "",
...
/* =========================
   THÊM
========================= */
window.addVanBan = async function () {
...
/* =========================
   LƯU
========================= */
window.saveVanBan = async function () {

  if (!editingId) {
    showToast("Chưa chọn văn bản để lưu");
    return;
  }
  ...
  await writeData(
    `teacher/${teacherId}/vanban/${editingId}`,
    {
      ...data,
      created_at: data.created_at ?? Date.now()
    }
  );
showToast("Đã lưu văn bản");
  clearForm();
  loadList();
};
...
async function loadList() {
  ...
  Object.entries(data).forEach(([id, item]) => {
    ...
    tbody.innerHTML += `
      <tr onclick="editVanBan('${id}')" style="cursor:pointer">
        <td>${i++}</td>
        <td>${item.title}</td>
        <td>${item.ngay || ""}</td>
        <td>
...
/* =========================
   EDIT
========================= */
window.editVanBan = async function (id) {
...
/* =========================
   DELETE
========================= */
window.deleteVanBan = async function (id) {
